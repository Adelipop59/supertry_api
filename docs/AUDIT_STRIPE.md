...
**H1** (réconciliation wallet DB ↔ solde Connect), **H2** (`escrowAmount` au paiement — à vérifier côté checkout/webhook), **H3** (idempotence create/confirm PI + lock — partiellement caduc côté « pay » supprimé, à valider sur le checkout), **H5** (workflow d'approbation retrait), **H7** (course catch/webhook — atténuée par C2+C4, à finaliser), **H8** (cohérence Identity/retrait), **M1** (double comptabilisation escrow — atténuée par C4, à revérifier), **M3** (whitelist URLs onboarding/identity), **M5/M6/M9**, **L2/L3**.
